...
void estun::ComputeRender::CopyImage(std::shared_ptr<ImageHolder> image1, std::shared_ptr<ImageHolder> image2)
{
...
void estun::ComputeRender::ImageBarrier(
    std::shared_ptr<ImageHolder> image,
...
{
    image->Barrier(GetCurrCommandBuffer(), newLayout, srcAccessMask, dstAccessMask, sourceStage, destinationStage);
}
